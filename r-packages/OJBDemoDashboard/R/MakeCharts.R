...
chart_r1c1 <- function() {
  ...
  print(ggplot(data=YearData, aes(x=yearID)) +
...
chart_r1c2 <- function(league) {
  ...
  print(ggplot(data=df, aes(x=team, y=opp)) + geom_tile(aes(fill=Avg)) +
    scale_fill_gradient(low = "red", high = "green") + xlab("Team") + ylab("Opponent"))
  dev.off()
}
...
chart_r1c3 <- function() {
  ...
  print(ggplot(data=TeamData2015, aes(x=Wins, y=RunDiff)) + geom_point() + geom_text_repel(aes(label=team), nudge_x=1) +
          xlab("Season Wins") + ylab("Total Run Differential"))
  dev.off()
}
...
chart_r2c1 <- function() {
  ...
  print(ggplot(data=wins, aes(x=TotalRuns)) + geom_bar() + xlab("Total Runs Scored") + ylab("# of games"))
  dev.off()
}
...
chart_r2c2 <- function() {
  ...
  print(ggplot(data=ps2, aes(x=reorder(label, -value), y=value, fill=var)) + geom_bar(stat="identity") +
          xlab("Player/Season") + ylab("# of Home Runs") + scale_fill_discrete(labels=c("Player HR", "95th %ile HR")) +
